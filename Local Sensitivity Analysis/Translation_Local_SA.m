...
p = [alpha_x alpha_y alpha_z delta_x delta_y delta_z alpha_g delta_g alpha_y_prime ...
    delta_y_prime lambda gamma_y gamma_z delta_xy delta_xz];
...
paralist = [{'alpha_x'},{'alpha_y'},{'alpha_z'},{'delta_x'},{'delta_y'},{'delta_z'},...
    ...
    {'lambda'},{'gamma_y'},{'gamma_z'},{'delta_{xy}'},{'delta_{xz}'}];

j = 1;
idx = [];
for k = 1:15 %%% Including a Loop for each Variable %%%
    ...
    for ii = [0.001:0.001:0.009 0.01:0.01:0.09 0.1:0.1:0.9 1:1:9 10:10:90 100:100:1000]
        ...
        xlabel('Time (min)')
        ylabel('GFP Output')        
        plot(All_t,All_x(:,7)*1e9,'LineWidth',2)
        hold on
        title(['\',paralist{k}])
