%%%%%
%%%%%
%%%%% Simulation with nominal parameter
%%%%%
%%%%%
clear all
close all
...
gamma_y = 5e3; %%% X binds to Py
gamma_z = 5e3; %%% X binds to Pz
...
p = [alpha_x alpha_y alpha_z delta_x delta_y delta_z alpha_g delta_g ...
    gamma_y gamma_z beta_hy1 nu alpha_y_prime delta_y_prime ];
...
paralist = [{'alpha_x'},{'alpha_y'},{'alpha_z'},{'delta_x'},{'delta_y'},{'delta_z'},...
    {'alpha_g'},{'delta_g'},{'gamma_y'},{'gamma_z'},{'beta_{hy1}'},{'nu'},{'alpha_{y,p}'},{'delta_{y,p}'}];

j = 1;
idx = [];
for k = 1:14 %%% Including a Loop for each Variable %%%
    ...
    for ii = [0.001:0.001:0.009 0.01:0.01:0.09 0.1:0.1:0.9 1:1:9 10:10:90 100:100:1000]
        ...
        [t,x] = ode23s(@HY_TY1_ODE,tspan,x0,options,p);
        L = length(tspan);
        All_x = x.*1e9;                  %%% convert from M to nM for visualization
        All_t = t./60;
        
        label_list = ["X mRNA"; "Y mRNA"; "Z mRNA"; "GFP"; "Py act"; "Pz act"];
        ...
        ylabel('GFP Output')
        hold on
        plot(All_t,All_x(:,4),'LineWidth',2)
        title(['\',paralist{k}])
        ...
        if locmax_gfp*delta_t(1) <= t_cutoff*delta_t(1) && gfp(end) <= 0.1*max_gfp
            ...
            %%% Pulse Quality
            pulse_q(k,j) = pulse_height(k,j)/(pulse_height(k,j) + ss_gain(k,j))
