...
% gamma_y = 6e4; %%% X binds to Py
% gamma_z = 1e4; %%% X binds to Pz
...
paralist = [{'alpha_x'},{'alpha_y'},{'alpha_z'},{'delta_x'},{'delta_y'},{'delta_z'},...
    {'alpha_g'},{'delta_g'},{'gamma_y'},{'gamma_z'},{'omega'}];

j = 1;
idx = [];
for k = 1%1:11 %%% Including a Loop for each Variable %%%
    ...
    for ii = 1%[0.001:0.001:0.009 0.01:0.01:0.09 0.1:0.1:0.9 1:1:9 10:10:90 100:100:1000]
        ...
        label_list = ["X mRNA"; "Y mRNA"; "Z mRNA"; "GFP"];
        
        figure(1)
%         subplot(3,4,k)
        xlabel('Time (min)')
        ylabel('GFP Output')
        plot(All_t,All_x(:,4)*1e9,'LineWidth',2)
        hold on
        title(['\',paralist{k}])
        %         plot(All_t./60,All_x(:,4),'LineWidth',2)
        %         legend('Alpha X')
...
% save hy1metricnoscale t_rise pulse_width pulse_height ss_gain
