...
labels = {'\alpha_X','\alpha_Y','\alpha_Z','\delta_X','\delta_Y','\delta_Z',...
    '\alpha_G','\delta_G','\beta_{TX}','\gamma_Y','\gamma_Z','\nu'};

paralist = [{'alpha_X'},{'alpha_Y'},{'alpha_Z'},{'delta_X'},{'delta_Y'},{'delta_Z'},...
    {'alpha_G'},{'delta_G'},{'beta_{TX}'},{'gamma_Y'},{'gamma_Z'},{'nu'}];


% Plotting Phase Plot

load txmetricupdate.mat
...
for i = 1:length(maxtrise)
    compasspolar(angle_para(i),maxtrise(i)./TRN,[0 0.4470 0.7410])
    hold on
    compasspolar(angle_para(i),mintrise(i)./TRN,[0.4660 0.6740 0.1880])
    hold on
end
hold on
polarplot(th,nominaltrise(1)/TRN*ones(1,length(th)),'r','LineWidth',2)
title('Rise time (min)')
pax = gca;
...
for i = 1:length(maxpulsewidth)
    compasspolar(angle_para(i),maxpulsewidth(i)./PWN,[0 0.4470 0.7410])
    hold on
    compasspolar(angle_para(i),minpulsewidth(i)./PWN,[0.4660 0.6740 0.1880])
    hold on
end
hold on
polarplot(th,nominalpulsewidth(1)/PWN*ones(1,length(th)),'r','LineWidth',2)
...
for i = 1:length(maxpulseheight)
    compasspolar(angle_para(i),maxpulseheight(i)./PHN,[0 0.4470 0.7410])
    hold on
    compasspolar(angle_para(i),minpulseheight(i)./PHN,[0.4660 0.6740 0.1880])
    hold on
end
hold on
polarplot(th,nominalpulseheight(1)/PHN*ones(1,length(th)),'r','LineWidth',2)
...
for i = 1:length(maxssgain)
    compasspolar(angle_para(i),maxssgain(i)./SSN,[0 0.4470 0.7410])
    hold on
    compasspolar(angle_para(i),minssgain(i)./SSN,[0.4660 0.6740 0.1880])
    hold on
end
hold on
polarplot(th,nominalssgain(1)/SSN*ones(1,length(th)),'r','LineWidth',2)
